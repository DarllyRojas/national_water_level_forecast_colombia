...
def home(request):
    ...
    stations_file = os.path.join(os.path.join(app.get_app_workspace().path), 'Selelcted_Stations_Colombia_WL.json')
...
def get_popup_response(request):
    ...
    try:
        ...
        '''Get Observed Data'''
        auth = HydroShareAuthBasic(username=app.get_custom_setting('username'),
                                   password=app.get_custom_setting('password'))
        hs = HydroShare(auth=auth)
        resource_id = app.get_custom_setting('hydroshare_resource_id')
        hs.setAccessRules(resource_id, public=True)

        url = 'https://www.hydroshare.org/resource/{0}/data/contents/Water_Level_Data/{1}.csv'.format(resource_id,
                                                                                                      codEstacion)
...
def get_hydrographs(request):
    ...
    try:
        ...
        '''Plotting Data'''
        observed_WL = go.Scatter(x=observed_df.index, y=observed_df.iloc[:, 0].values, name='Observed',
                                 line=dict(color="#636EFA"))
        corrected_WL = go.Scatter(x=corrected_df.index, y=corrected_df.iloc[:, 0].values, name='Corrected Simulated',
        ...
        return render(request, 'national_water_level_forecast_colombia/gizmo_ajax.html', context)
...
def get_dailyAverages(request):
    ...
    try:
        ...
        daily_avg_obs_WL = go.Scatter(x=daily_avg2.index, y=daily_avg2.iloc[:, 1].values, name='Observed',
                                      line=dict(color="#636EFA"))
...
def get_monthlyAverages(request):
    ...
    try:
        ...
        monthly_avg_obs_WL = go.Scatter(x=monthly_avg2.index, y=monthly_avg2.iloc[:, 1].values, name='Observed',
                                        line=dict(color="#636EFA"))
...
def get_time_series_bc(request):
    ...
    try:
        ...
        if startdate != '':
            res = requests.get(
                'https://geoglows.ecmwf.int/api/ForecastEnsembles/?reach_id=' + comid + '&date=' + startdate + '&return_format=csv',
                verify=False).content
        else:
            res = requests.get(
                'https://geoglows.ecmwf.int/api/ForecastEnsembles/?reach_id=' + comid + '&return_format=csv',
                verify=False).content
        ...
        for mes in meses:
            ...
            for element in min_index_value:
                min_factor[column_records].loc[min_factor.index == element] = tmp[column_records].loc[
                                                                                  tmp.index == element] / min_simulated
            ...
            for element in max_index_value:
                max_factor[column_records].loc[max_factor.index == element] = tmp[column_records].loc[
                                                                                  tmp.index == element] / max_simulated
            ...
            min_factor_records_df.update(
                pd.DataFrame(min_factor[column_records].values, index=min_factor.index, columns=[column_records]))
            max_factor_records_df.update(
                pd.DataFrame(max_factor[column_records].values, index=max_factor.index, columns=[column_records]))
        ...
        url_rt = 'http://fews.ideam.gov.co/colombia/jsonH/00' + codEstacion + 'Hobs.json'
        ...
        if f.status_code == 200:
            ...
            try:
                ...
                observed_rt = pd.DataFrame(pairs, columns=['Datetime', 'Observed Water Level (cm)'])
                observed_rt.set_index('Datetime', inplace=True)
                observed_rt["Observed Water Level (cm)"] = 100 * observed_rt["Observed Water Level (cm)"]
                ...
                observed_rt_plot = observed_rt_plot.loc[
                    observed_rt_plot.index >= pd.to_datetime(forecast_ens.index[0] - dt.timedelta(days=8))]
                observed_rt_plot = observed_rt_plot.loc[
                    observed_rt_plot.index <= pd.to_datetime(forecast_ens.index[0] + dt.timedelta(days=2))]
            ...
            try:
                ...
                sensor_rt = pd.DataFrame(pairs, columns=['Datetime', 'Sensor Water Level (cm)'])
                sensor_rt.set_index('Datetime', inplace=True)
                sensor_rt["Sensor Water Level (cm)"] = 100 * sensor_rt["Sensor Water Level (cm)"]
                ...
                sensor_rt_plot = sensor_rt_plot.loc[
                    sensor_rt_plot.index >= pd.to_datetime(forecast_ens.index[0] - dt.timedelta(days=8))]
                sensor_rt_plot = sensor_rt_plot.loc[
                    sensor_rt_plot.index <= pd.to_datetime(forecast_ens.index[0] + dt.timedelta(days=2))]
...
def get_available_dates(request):
    ...
    res = requests.get('https://geoglows.ecmwf.int/api/AvailableDates/?region=' + watershed + '-' + subbasin,
                       verify=False)
...
def get_simulated_bc_water_level_csv(request):
    ...
    try:
        ...
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = 'attachment; filename=corrected_simulated_water_level_{0}.csv'.format(
            codEstacion)
...
def get_forecast_bc_data_csv(request):
    ...
    try:
        ...
        response = HttpResponse(content_type='text/csv')
        response[
            'Content-Disposition'] = 'attachment; filename=corrected_water_level_forecast_{0}_{1}_{2}_{3}.csv'.format(
...
def get_forecast_ensemble_bc_data_csv(request):
    ...
    try:
        ...
        response = HttpResponse(content_type='text/csv')
        response[
            'Content-Disposition'] = 'attachment; filename=corrected_water_level_ensemble_forecast_{0}_{1}_{2}_{3}.csv'.format(
            watershed, subbasin, comid, startdate)
